...
struct BasicInfo {
    ...
    BasicInfo(const std::deque<Domain::Bus>& buses_, const std::deque<Domain::Stop>& stops_) {
        buses = buses_;
        stops = stops_;
        for (auto bus : buses)
        {
...
void AddRoutsNames(BasicInfo& basic_info, svg::Document& to_draw, RenderSettings& settings) {
    BasicInfo info = basic_info;
    int color_id = 0;
    const SphereProjector proj(info.raw_cords.begin(), info.raw_cords.end(), settings.width, settings.height, settings.padding);
...
void AddStopPoints(BasicInfo& basic_info, svg::Document& to_draw, RenderSettings& settings) {
    BasicInfo info = basic_info;
    const SphereProjector proj(info.raw_cords.begin(), info.raw_cords.end(), settings.width, settings.height, settings.padding);
...
void AddStopNames(BasicInfo& basic_info, svg::Document& to_draw, RenderSettings& settings) {
    BasicInfo info = basic_info;
    const SphereProjector proj(info.raw_cords.begin(), info.raw_cords.end(), settings.width, settings.height, settings.padding);
    for (const auto& stop : info.stops)
    {
...
std::ostringstream MapRender::Draw(const std::deque<Domain::Bus>& buses, const std::deque<Domain::Stop>& stops) {
