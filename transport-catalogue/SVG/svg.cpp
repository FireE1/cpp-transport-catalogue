...
inline void PrintColor(std::ostream &out, Rgb &rgb) {
  out << "rgb("sv << static_cast<short>(rgb.red) << ","sv
      << static_cast<short>(rgb.green) << ","sv << static_cast<short>(rgb.blue)
      << ")"sv;
}
...
inline void PrintColor(std::ostream &out, Rgba &rgba) {
  out << "rgba("sv << static_cast<short>(rgba.red) << ","sv
      ...
      << static_cast<short>(rgba.blue) << ","sv << (rgba.alpha) << ")"sv;
}

inline void PrintColor(std::ostream &out, std::monostate) { out << "none"sv; }

inline void PrintColor(std::ostream &out, std::string &color) { out << color; }
...
void Text::RenderObject(const RenderContext &context) const {
  ...
  out << "<text";
  RenderAttrs(out);
  out << R"( x=")" << position_.x << R"(" y=")" << position_.y << R"(" dx=")"sv
      << offset_.x << R"(" dy=")" << offset_.y << R"(" font-size=")"
      << font_size_;
