...
public:
    ...
    void SaveBus(std::string& bus, bool is_round) {name_of_buses.insert(std::make_pair(bus, is_round));}
    void SaveStop(std::string stop) {name_of_stops.insert(stop);}
    std::set<std::pair<std::string, bool>>& GetAllBuses() {return name_of_buses;}
