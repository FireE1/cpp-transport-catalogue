...
const std::set<std::string_view> TransporCatalogue::GetRoutsOnStop(std::string_view stop_name) const {
    // Пустое значение для выдачи финального результата функции
...
void TransporCatalogue::SetStopToStopDistance(std::string_view stop_from, std::string_view stop_to, size_t lenght) {
