...
void SerializationClass::SerializeCatalogue(
...
void SerializationClass::SerializeRenderSettings(
  ...
  *serialized_info_.mutable_map_render_settings() =
      std::move(serialized_settings);
}

void SerializationClass::SerializeRouting(
    const TransportRouter::Router &routing) {
  ...
  for (const auto &[edge, edge_type] : routing.FastEdgeGetGetter()) {
    ...
    if (std::holds_alternative<TransportRouter::StopEdge>(edge_type)) {
      TransportRouter::StopEdge stop_edge =
          std::get<TransportRouter::StopEdge>(edge_type);
      ...
      *serialized_edge_type.mutable_stop_edge() = serialized_stop_edge;
      ...
      TransportRouter::BusEdge bus_edge =
          std::get<TransportRouter::BusEdge>(edge_type);
...

  {
    ...
    const auto &graph_incidence_lists =
        routing.GraphGetter()->GetIncidenceLists();
      ...
      for (const auto &internal : rts_vect) {
        ...
        if (internal.has_value()) {
          ...
          if (internal.value().prev_edge.has_value()) {
            ...
            serialized_internal.set_prev_edge(
                internal.value().prev_edge.value());
          } else {
        ...
        *serialized_rts_vect.add_route_internal_data_vector() =
            serialized_internal;
      }
...
CatalogueCore::TransporCatalogue SerializationClass::DeserializeCatalogue() {
  ...
  }
  {
    ...
    for (const auto &serialized_distance : serialized_distances) {
      std::string from =
          serialized_stops[serialized_distance.stop_from()].stop_name();
      std::string to =
          serialized_stops[serialized_distance.stop_to()].stop_name();
  ...
  }
  {
    ...
    for (const auto &serialized_bus : serialized_buses) {
      ...
      for (const auto &bus_stop : serialized_bus.stops()) {
        bus.stops_for_bus_.push_back(
            cat.FindStop(serialized_stops[bus_stop].stop_name()));
      }
...
svg::Color
DeserializeColor(const proto_transport_svg::Color &serialized_color) {
  svg::Color color;
...
map_render::RenderSettings SerializationClass::DeserializeRenderSettings() {
  ...
  {
    const auto &serialized_bus_label_offset =
        serialized_settings.bus_label_offset();
  ...
  {
    const auto &serialized_stop_label_offset =
        serialized_settings.stop_label_offset();
...
TransportRouter::Router SerializationClass::DeserializeRouting(
    const CatalogueCore::TransporCatalogue &cat) {
  // получаем настройки автобуса
  const auto &serialized_routing = serialized_info_.router();
  TransportRouter::BusW_TimeAndVelocity routing_settings = {
      serialized_routing.settings().wait_time(),
      serialized_routing.settings().velocity()};
  ...
  {
    std::vector<
        std::pair<const Domain::Stop *, TransportRouter::RouterStopWait>>
        to_insert_sw;
    for (const auto &stop_wait : serialized_routing.stop_wait()) {
      proto_transport_router::RouterStopWait serialized_rsw =
          stop_wait.stop_wait();
      TransportRouter::RouterStopWait begin_end = {serialized_rsw.begin(),
                                                   serialized_rsw.end()};
      to_insert_sw.push_back(
          std::make_pair(cat.FindStop(stop_wait.stop()), begin_end));
    }
    routing.MutableStopsWaitGetter().insert(to_insert_sw.rbegin(),
                                            to_insert_sw.rend());
  }
  // вкидываем быстрое получение edge
  {
    std::vector<
        std::pair<graph::EdgeId, std::variant<TransportRouter::StopEdge,
                                              TransportRouter::BusEdge>>>
        to_insert_feg;
    ...
    routing.MutableFastEdgeGetGetter().insert(to_insert_feg.rbegin(),
                                              to_insert_feg.rend());
  }
  // разбираемся с graph
  {
    ...
    // graph incidence lists
    {
      ...
      for (const auto &serialized_grp_incidence_list :
           serialized_routing.graph().incidence_lists()) {
        std::vector<size_t> incodence_list;
        for (const auto &serialized_incedence :
             serialized_grp_incidence_list.incidence_list()) {
  ...
  routing.MutableRouterGetter() = std::move(
      std::make_unique<graph::Router<double>>(*routing.GraphGetter()));
  int outer_vector_id = 0;
  for (const auto &serialized_internal_st1 : serialized_routing.router()
                                                 .routes_internal_data()
                                                 .routes_internal_data()) {
    int inner_vector_id = 0;
    for (const auto &serialized_internal :
         serialized_internal_st1.route_internal_data_vector()) {
      if (!serialized_internal.nullopt()) {
        if (serialized_internal.prev_edge_set()) {
          ...
          *routing.MutableRouterGetter()->SetInternal(outer_vector_id,
                                                      inner_vector_id) = to_add;
        } else {
          double weight = serialized_internal.weight();
          graph::Router<double>::RouteInternalData to_add;
          to_add.weight = weight;
