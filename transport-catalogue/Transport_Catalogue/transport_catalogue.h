...
public:
  ...
  GetRoutsOnStop(std::string_view stop_name) const;

  // Функция добавления реальной дистанции(по дорогам) для остановок (остановка
  // -> {остановка -> расстояние})
  void SetStopToStopDistance(std::string_view stop_from,
                             std::string_view stop_to, size_t lenght);

  size_t GetDistanceBetweenStops(const Domain::Stop *from,
...
private:
  ...
  // Хранилище реальной дистанции(по дорогам) между остановками (остановка ->
  // {остановка -> расстояние})
